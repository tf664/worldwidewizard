<script lang="ts">
	...
	export let onPlayerElementsReady: (elements: HTMLElement[]) => void = () => {}; // Add this export
	...
	// Notify parent when all player elements are ready
	$: if (playerElements.length === gameState.players.length && playerElements.every((el) => el)) {
		onPlayerElementsReady(playerElements);
	}
	...
	function isCurrentPlayer(player: Player): boolean {
		...
		const currentTurnPlayerIndex = gameState.currentPlayerIndex;
		const playerIndex = gameState.players.findIndex((p) => p === player);
		return playerIndex === currentTurnPlayerIndex;
	}
	...
	function getPlayerCircularPosition(
		...
		const isTopPlayer = y < -(windowHeight * 0.3); // Adjust threshold as needed
		const isBottomPlayer = y > windowHeight * 0.3;
		...
		if (isTopPlayer) {
			// Ensure top players don't go off-screen when scaled
			y = Math.max(y, -(windowHeight * 0.45) + 60); // Add buffer for scaled card
		} else if (isBottomPlayer) {
			// Ensure bottom players don't go off-screen when scaled
			y = Math.min(y, windowHeight * 0.45 - 60); // Add buffer for scaled card
		}
